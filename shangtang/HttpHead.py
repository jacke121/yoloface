...
class HttpRequest(object):
    ...
    def passRequest(self, request):
        ...
        if self.method == 'POST':
            ...
            try:
                if "recogResult" in request_body:
                    feature = True
                    if len(request_body)<3000:
                        print("data lost")
                    else:
                        request_data = json.loads(request_body)#.split("feature")[0][:-2] + "}}")
                        print("feat",request_data["recogResult"]["similars"])
                    # print(request_body.split("feature")[1][2:].split("frameIdx")[0][:-2])
                    # print("frameIdx"+request_body.split("feature")[1][2:].split("frameIdx")[1])
                else:
                    feature = False
                    request_data = json.loads(request_body)  # 每一行是一个字段

                    # recogResult
                    if "captureResult" in request_data:
                        print("captureResult")
                    elif "endObjs" in request_data:
                        print("track endObjs")
                    elif "attrResult" in request_data:
                        print("属性 attrResult")
                    else:
                        print(request_body)
            except Exception as e:
                print(feature, "json error", e)
                if feature:
                    print(request_body.split("feature")[0][:-2] + "}}")
